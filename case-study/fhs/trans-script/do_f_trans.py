...
def process_file_f(filename, module_name):
    ...
    # print("injector_obj:",injector_obj)
    # print("fault_ls:",fault_ls)
...
def trans_protocol(filename, protocol_name, injector_obj, fault_ls):
        ...
        if line2.find("debugFlagM") >= 0:
            ...
            for fault_abbr in fault_module_map.keys():
                if fault_abbr in fault_ls:
                    tmp2.append(fault_number_map[fault_abbr])
                else:
                    tmp2.append("0.0")
            tmp2 = '(' + ",".join(tmp2) + ')'
    ...
    while j < len(ftxt2):
        line2 = ftxt2[j]
        if (line2.find(" var ") >= 0 or line2.find(" vars ") >= 0) and line2.find("T : Float") < 0:
    ...
    # print("injector_left_lines:",injector_left_lines)
    # print("injector_right_lines:",injector_right_lines)
    # print("evil_cond_lines:",evil_cond_lines)
                ...
                while i <= rl_stmt_end_line_n:
                    ...
                    if rl_flag == 'n' and (line.find("{") >= 0 or line.find("<") >= 0):
                        ...
                        for line_evil in injector_left_lines:
                            copy_flag = True
                            for fault_abbr_comp in fault_ls_comp:
                                # print(">>>",fault_abbr_comp)
                                if line_evil.find(" "+fault_abbr_comp.upper()) >= 0:
                                    copy_flag = False
                                    break
                            # print(">left evil line:",line_evil, copy_flag)
                            if copy_flag:
                                fwp.write(line_evil)
                        rl_flag = 'le'
                        # print(">> l to le")
                    ...
                    if rl_flag == 'c':
                        ...
                        for line_evil in evil_cond_lines:
                            copy_flag = True
                            for fault_abbr_comp in fault_ls_comp:
                                if line_evil.find(fault_abbr_comp.upper()+"notFault") >= 0:
                                    copy_flag = False
                                    break
                            if copy_flag:
